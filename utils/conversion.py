...
def paired_random_crop_prior_0(img_gts, img_lqs, img_preds, img_mvs, img_residues, gt_patch_size, gt_path, scale=1):
    """Paired random crop.
    It crops lists of lq and gt images with corresponding locations.
    Args:
        img_gts (list[ndarray] | ndarray): GT images. Note that all images
    ...
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_lqs, img_preds, img_mvs, img_residues
...
def paired_random_crop_prior_res(img_gts, img_residues, img_lqs, img_preds, img_mvs, gt_patch_size, gt_path, scale=1):
    """Paired random crop.
    It crops lists of lq and gt images with corresponding locations.
    Args:
        img_gts (list[ndarray] | ndarray): GT images. Note that all images
    ...
    if not isinstance(img_gts, list):
        img_gts = [img_gts]
    if not isinstance(img_residues, list):
        img_residues = [img_residues]
    if not isinstance(img_lqs, list):
        img_lqs = [img_lqs]
    ...
    if len(img_gts) == 1:
        img_gts = img_gts[0]
    if len(img_residues) == 1:
        img_residues = img_residues[0]
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_residues, img_lqs, img_preds, img_mvs





def paired_random_crop_prior_mv(img_gts, img_lqs, img_mvs, gt_patch_size, gt_path, scale=1):
    if not isinstance(img_gts, list):
        img_gts = [img_gts]
    if not isinstance(img_lqs, list):
        img_lqs = [img_lqs]
...
def paired_random_crop_prior_mvpred(img_gts, img_lqs, img_preds, img_mvs, gt_patch_size, gt_path, scale=1):
    """Paired random crop.
    It crops lists of lq and gt images with corresponding locations.
    Args:
        img_gts (list[ndarray] | ndarray): GT images. Note that all images
    ...
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts, img_lqs, img_preds, img_mvs
...
def paired_random_crop_prior_0res(img_gts, img_lqs, img_preds, img_mvs, gt_patch_size, gt_path, scale=1):
    """Paired random crop.
    It crops lists of lq and gt images with corresponding locations.
    Args:
        img_gts (list[ndarray] | ndarray): GT images. Note that all images
