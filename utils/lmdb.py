...
def make_lmdb_from_imgs(img_dir,
    ...
    if multiprocessing_read:
        ...
        for path, findex, key in zip(img_path_list, index_frame_list, keys):
            # print('[path]', op.join(path,'{:03d}.png'.format(findex)))
            pool.apply_async(
                _read_img_worker,
                args=(op.join(path,'{:03d}.png'.format(findex)), key, compress_level),  #  img_dir
                callback=_callback
                )
        pool.close()
        pool.join()
    ...
    if map_size is None:
        ...
        # print('[paaa]', op.join(img_path_list[0],keys[0]))
        # op.join(path,'{:03d}.png'.format(findex)
        img = cv2.imread(
            op.join(img_path_list[0],'001.png'), cv2.IMREAD_UNCHANGED)
    ...
    pbar = tqdm(total=num_img, ncols=80)
    for idx, (path, findex, key) in enumerate(zip(img_path_list, index_frame_list, keys)):
    # for idx, (path, key) in enumerate(zip(img_path_list, keys)):
        ...
        _, img_byte, img_shape = _read_img_worker(
            op.join(path,'{:03d}.png'.format(findex)), key, compress_level 
            )  # use _read function
        h, w, c = img_shape
...
def make_lmdb_from_npys(npy_dir,
                        lmdb_path,
                        npy_path_list,
                        index_frame_list,
                        keys,
    ...
    """
    # check
    assert len(npy_path_list) == len(keys), (
        'npy_path_list and keys should have the same length, '
        f'but got {len(npy_path_list)} and {len(keys)}')
    assert lmdb_path.endswith('.lmdb'), "lmdb_path must end with '.lmdb'."
    assert not op.exists(lmdb_path), f'Folder {lmdb_path} already exists. Exit.'

    # display info
    num_npy = len(npy_path_list)

    # read all the images to memory by multiprocessing
    if multiprocessing_read:
        def _callback(arg):
            """Register npy and shapes into the dict & update pbar."""
            key, npy_byte, npy_shape = arg
            dataset[key], shapes[key] = npy_byte, npy_shape
            pbar.set_description(f'Read {key}')
            pbar.update(1)
        
        dataset = {}  # use dict to keep the order for multiprocessing
        shapes = {}
        pbar = tqdm(total=num_npy, ncols=80)
        pool = Pool()  # default: cpu core num
        # read an image, and record its byte and shape into the dict 
        # for path, findex, key in zip(img_path_list, index_frame_list, keys):
        #     print('[path]', op.join(path,'{:03d}.png'.format(findex)))
        for path, findex, key in zip(npy_path_list, index_frame_list, keys):
            pool.apply_async(
                _read_npy_worker,
                args=(op.join(path,'{:03d}.npy'.format(findex)), key, compress_level),
                callback=_callback
                )
        pool.close()
        pool.join()
    ...
    # estimate map size if map_size is None
    if map_size is None:
        # obtain data size for one npy
        y_pyt = np.load(op.join(npy_path_list[0],'001.npy')).astype(np.float32)
        # y_pyt = np.expand_dims(np.expand_dims(res_map, axis=0), axis=0).astype(np.float32)

        data_size_per_npy = y_pyt.nbytes
        data_size = data_size_per_npy * len(npy_path_list)
        map_size = data_size * 10  # enlarge the estimation
    ...
    txt_file = open(op.join(lmdb_path, 'meta_info.txt'), 'w')
    pbar = tqdm(total=num_npy, ncols=80)
    for idx, (path, findex, key) in enumerate(zip(npy_path_list, index_frame_list, keys)):
        pbar.set_description(f'Write {key}')
        pbar.update(1)

        # load image bytes
        if multiprocessing_read:
            npy_byte = dataset[key]  # read from prepared dict
            h, w, c = shapes[key]
        else:
            _, npy_byte, npy_shape = _read_npy_worker(
                op.join(path,'{:03d}.npy'.format(findex)), key, compress_level
                )  # use _read function
            h, w, c = npy_shape

        # print('[npy_shape]',npy_shape)
        # write lmdb
        key_byte = key.encode('ascii')
        txn.put(key_byte, npy_byte)
...
def make_lmdb_from_MVnpys(npy_dir,
                        lmdb_path,
                        npy_path_list,
                        index_frame_list,
                        keys,
...
def _read_npy_worker(path, key, compress_level):
    """Read npy worker.
    Args:
        path (str): npy path.
        key (str): npy key.
    ...
    """
    npdata = np.load(path)
    npdata = npdata.transpose(1,2,0)
    if npdata.ndim == 2:
        h, w = npdata.shape
        c = 1
    else:
        h, w, c = npdata.shape
    npdata_byte = npdata.tobytes()
    return (key, npdata_byte, (h, w, c))
...
def make_y_lmdb_from_yuv400(
        video_path_list, index_frame_list, key_list, lmdb_path, 
        yuv_type='400p', h=None, w=None, 
        batch=7000, compress_level=1, multiprocessing_read=False, map_size=None
        ):
    # check
    assert lmdb_path.endswith('.lmdb'), "lmdb_path must end with '.lmdb'."
    ...
    for iter_frm in range(num_img):
        # print('{iter_frm}',iter_frm)
        # print('{video_path_list[iter_frm]}',video_path_list[iter_frm]) 
        # print("[index_frame_list[iter_frm]]",index_frame_list[iter_frm])
        pool.apply_async(
            _read_y_from_yuv_worker,
            args=(
                video_path_list[iter_frm], 
    ...
    if map_size is None:
        ...
        for iter_img in range(num_img):
            ...
            if w == None:
                # w, h = map(int, vid_path.split('.')[-2].split('_')[-2].split('x'))
                w = int((vid_path.rsplit('x',1)[0]).split('_')[-1])
                h = int((vid_path.rsplit('x',1)[1]).split('_')[0])
            img_size = w * h
            if img_size > biggest_size:
                biggest_size = img_size
                biggest_index = iter_img
